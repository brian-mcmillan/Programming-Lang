...
; see line 435 and 509 for 4.40 and 4.42 ;
...
(define the-grammar
  '((program (expression) a-program)
    ...
    (expression
     ("lazy" identifier "=" expression "in" expression)
     lazy-exp)   
...
(eval "lazy x = 7 in x")
(eval "lazy p = newpair(10, 11)
       in left(p)")
(eval "lazy p = newpair(10, 11)
       in right(p)")
(eval "lazy g = proc (x) x in (g 50)")
(eval "lazy f = proc (x) x in (f 1000)")
(eval "lazy g = let count = 0
               in proc (dummy)
                    begin
                      set count = +(count, 1);
                      count
...
(eval "let x = 7 in x")
(eval "let p = newpair(10, 11)
       in left(p)")
(eval "let p = newpair(10, 11)
       in right(p)")
(eval "let g = proc (x) x in (g 50)")
(eval "let f = proc (x) x in (f 1000)")
(eval "let g = lazy count = 0
               in proc (dummy)
                    begin
                      set count = +(count, 1);
                      count
